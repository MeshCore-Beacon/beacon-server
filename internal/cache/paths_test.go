...
func TestPathCacheSeparatesWindowAndRegion(t *testing.T) {
	c, mr := newTestClient(t)
	inner := &pathStatsReader{}
	...
	for _, tc := range []struct {
		...
		got, err := r.GetPathStats(context.Background(), time.UnixMilli(tc.since), time.UnixMilli(tc.until), tc.iatas)
		if err != nil || got.Receptions != tc.want || !slices.Equal(before, tc.iatas) {
			t.Fatalf("%+v: %+v %v", tc, got, err)
		}
	}
	mr.FastForward(time.Minute)
	got, err := r.GetPathStats(context.Background(), time.UnixMilli(0), time.UnixMilli(10), nil)
